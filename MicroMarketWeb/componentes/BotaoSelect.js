...
export default ({options=[], onChange , initialSelect=[], titulo = " ", max, selecionado, select}) => {
    ...
    function renderItem(item){
        return (
            ...
            onPress={()=> combinada(item)}>
                <Text style={[Estilo.texto12px, Estilo.textoCorPrimaria, style.Montserrat]}>{item}</Text>
            </TouchableOpacity>
        )
    }
    ...
    return (
        ...
        >
            <Text style={ selecionado ?[Estilo.textoCorPrimaria, style.actions, Estilo.texto12px, style.Montserrat] :  [Estilo.textoCorDanger, style.actions, Estilo.textoP16px, style.Montserrat]}> {selected == '' ? select ? select : 'Select' : selected}</Text>
                ...
                style={{flex: 1}}>
                    <View style={style.header}>
                        <View style={style.headerR1}>
                            <View>
                                <Text style={[Estilo.textoCorPrimaria, style.actions, Estilo.tituloH619px, style.Montserrat]}>{titulo || 'Título'}</Text>
                                <Text style={[Estilo.textoCorPrimaria, style.actions, Estilo.textoP16px, style.Montserrat]}>{`Selecione ${max} opção`}</Text>
                            </View>
